...
bign bign::operator=(const char *num) {
    ...
    if (num[0] != '-')
        ...
        for (int i = 1, len = strlen(num); i < len; i++) a[i - 1] = num[i];
    sign = !(num[0] == '-');
    len = strlen(a);
    for (int i = 0, len = strlen(a); i < len; i++) s[i] = a[len - i - 1] - 48;
    return *this;
}
...
bign solve(int& l, int& r)
{
    bign res = 0, tmp = 0;
    
    for(; l < r; ++l)
    {
        if(str[l] == '(') tmp = pow(tmp, solve(++l, r));
        else if(str[l] == ')') {res += tmp; tmp = 0; break;}
        else if(str[l] == '+') {res += tmp; tmp = 0;}
        else tmp = str[l] ^ 48;
    }
    return res + tmp;
}
...
{
    scanf("%s", str);
    int beg = 0, end = strlen(str);
    bign ans = solve(beg, end);
    cout << ans << endl;
    return 0;
}